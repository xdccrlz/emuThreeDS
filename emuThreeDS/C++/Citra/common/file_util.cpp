...
std::string GetSysDirectory() {
    ...
    LOG_INFO(Common_Filesystem, "bundle = {}, document = {}", GetBundleDirectory(), DirectoryManager::DocumentsDirectory());
    
#if defined(__APPLE__)
    sysDir = DirectoryManager::DocumentsDirectory(); // GetBundleDirectory();
    sysDir += DIR_SEP;
    sysDir += SYSDATA_DIR;
#else
    sysDir = SYSDATA_DIR;
    ...
    if (!path.empty() && CreateFullPath(path)) {
        ...
        if (FileUtil::Exists(ROOT_DIR DIR_SEP USERDATA_DIR)) {
            ...
            if (!FileUtil::Exists(data_dir) && !FileUtil::Exists(config_dir) &&
                ...
                data_dir = std::string(DirectoryManager::DocumentsDirectory()) + DIR_SEP;
